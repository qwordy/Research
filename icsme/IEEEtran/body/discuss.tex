...
\noindent
\textbf{The extension of existing tools.} Our patterns and findings can help improving existing program transformation tools. For example,  Lin \emph{et al.}~\cite{conf/sigsoft/LinRD14} proposed Asynchronizer that transforms self-written code with the \CodeIn{AsyncTask} class. Our study finds self-written code snippets that can be replaced with other parallel API classes. With minor modifications, we can extend Asynchronizer to transform self-written code to more APIs. We plan to explore such extension in future work.
