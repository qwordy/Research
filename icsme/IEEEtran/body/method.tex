...
\textbf{RQ1. Determining change patterns.} To explore this research question, we manually analyze \zhong{all the?} concurrency-related commits. For example, the below is a concurrency-related commit. The top five lines describe the metadata of the commit. The other lines describe the differences between two versions of code.
...
\textbf{RQ2. Exploring the usefulness of our change patterns.} We search the keywords of pattern in repositories. We search code in Github with a group of keywords of a pattern. We find numerous code pieces in the search results. We manually check the code and decide whether it is acceptable for the pattern. If it is, we fork the project, make our changes, commit and submit the pull request. %similar answer?%\zhong{Please explain how to build queries according to your patterns. Please explain how to you generate your patches?}

\textbf{RQ3. Determining the trends of using parallel APIs.} We count the usages of parallel APIs in the interval of months. To explore this research question, we analyze the trend of parallel APIs during the software evolution. In particular, we count the occurrence of 53 API classes from the \CodeIn{java.util.concurrent} package. The \CodeIn{java.util.concurrent} package is a frequently-used package when developers write concurrent programs. The popularity of them span a broad range. For each parallel API class, we draw a figure to denote its trend of modifications over time. Based on the shapes of such figures, we put parallel API classes into different categories. We define that a class is related to a commit if and only if the class occurs in added lines or deleted lines of the commit.%  \zhong{How did you determine that a parallel class is related to a commit?}

\textbf{RQ4. Determining the correlations between commits and concurrency-related commits} We count the number of commits and concurrency-related commits in each month respectively. We use Spearman's rank correlation coefficient to measure how two variables are correlated. The result is positive. The average coefficient is 0.726.%\zhong{Please explain what measure do you use to determine the correlations}
