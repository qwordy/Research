...
We have found some contexts which are suitable for the change patterns in different projects. We make 3 pull requests. One is accepted. One is abandoned. One is not responded yet.

Schmince-2\footnote{\url{https://github.com/derekmu/Schmince-2}} is a game in Google Play. It has the following code:
...
UnifiedEmail\footnote{\url{https://github.com/HexagonRom/android_packages_apps_UnifiedEmail}} is an open-source android application in Github. It has the following code:
...
The method returns a map. If the map does not exist, the method will create one. However, synchronizing the whole method is too heavy. We can synchronize only when the variable is not null and hence improve the performance. The modified code is as follow:
...
Spider4java\footnote{\url{https://github.com/lichangshu/spider4java}} is a crawler tool written in Java. It has the following code:
...
\begin{figure*}
	\centering
	\subfigure[Ascending]{\includegraphics[width=2.3in]{CountDownLatch}}
...
We calculate Spearman's rank correlation coefficient of number of commits and concurrency commits. The correlation should be between -1 and +1. The value +1 denotes that it is a perfect monotone function of two variables. The correlations are marked in the Figure~\ref{fig:confidence}. Our result in a different language is similar to Gu's result. Our result is consistent with Gu's results except one project, Cassandra, with a correlation of 0.094.
